...
def plot_bboxes_on_image_pos(image_path, df, grayscale_image, output_path,prediction_class):
    ...
    for index, row in df.iterrows():
        ...
        if nonzero_percentage > 0.1:
            ...
            plt.text(abs_xmin, abs_ymin - 10, text, color='black', fontsize=15, bbox=dict(facecolor='white', alpha=0.5))
...
def predict():
    ...
    if request.method == 'POST':
        ...
        elif model == model_7_14 and prediction_class == 3:
                shap_values = explainer7_14_age.shap_values(reshaped_user_uploaded_image)       
        elif model == model_15_23 and prediction_class == 2:
                shap_values = explainer15_23_gender.shap_values(reshaped_user_uploaded_image)
